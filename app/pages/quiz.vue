...
<template>
  ...
  >
    <p class="text-2xl mb-4 text-center text-gray-600">
      Level {{ parsedQueryParams.level }}
    </p>

    <div v-if="text !== undefined" class="animate-fade-in w-full max-w-3xl">
      <article class="rounded-xl bg-white p-8 shadow-lg border">
        <div class="mb-8 space-y-6">
          <MemoryLevelMeter :memory-level="text.memoryLevel" />
          <p
            class="text-pretty text-center text-xl text-gray-800 sm:text-2xl md:text-3xl"
          >
            {{ text.en }}
          </p>
          <div
